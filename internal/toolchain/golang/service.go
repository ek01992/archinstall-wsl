...
func (s *Service) Install() error {
	...
	cur, err := s.current()
	if err != nil {
		if err := s.r.Run("pacman", "-S", "--noconfirm", "go"); err != nil {
	...
	if err := s.r.Run("pacman", "-Syu", "--noconfirm", "go"); err != nil {
	...
	if cur2 != latest {
		logx.Error("toolchain: go verify failed", "expected", latest, "got", cur2)
		return fmt.Errorf("verification failed: expected %s, got %s", latest, cur2)
	}
	return nil
}
