...
func (s *Service) current() (string, error) {
	...
	if err != nil {
		if out2, err2 := s.r.Shell("node -v"); err2 == nil {
			out = out2
		} else {
			return "", err
		}
	...
	if !reNode.MatchString(ver) {
		return "", fmt.Errorf("unable to parse node -v output: %q", ver)
	}
	return ver, nil
}

func (s *Service) Install() error {
	logx.Info("toolchain: ensure node LTS")
	...
	if _, err := s.r.Output("nvm", "--version"); err != nil {
		if err := s.r.Run("pacman", "-S", "--noconfirm", "nvm"); err != nil {
	...
	if err != nil || cur != lts {
		if err := s.r.Run("nvm", "install", lts); err != nil {
			if _, err2 := s.r.Shell("source /usr/share/nvm/init-nvm.sh 2>/dev/null || true; nvm install " + lts); err2 != nil {
				logx.Warn("toolchain: node install via shell fallback failed", "err", err, "err2", err2)
		...
		if err := s.r.Run("nvm", "alias", "default", lts); err != nil {
			if _, err2 := s.r.Shell("source /usr/share/nvm/init-nvm.sh 2>/dev/null || true; nvm alias default " + lts); err2 != nil {
				logx.Warn("toolchain: node alias via shell fallback failed", "err", err, "err2", err2)
		...
		if nv != lts {
			logx.Error("toolchain: node verify failed", "expected", lts, "got", nv)
