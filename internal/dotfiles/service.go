...
`
		...
		if err := s.fs.WriteFile(z, []byte(defaultContent), 0o644); err != nil {
			return fmt.Errorf("write default .zshrc: %w", err)
		}
		logx.Info("dotfiles: wrote default .zshrc", "path", z)
		return nil
	}
	repoDir := filepath.Join(home, ".dotfiles")
	if _, err := s.fs.Lstat(repoDir); err != nil {
		logx.Info("dotfiles: clone repository", "url", repoURL, "dest", repoDir)
		if err := s.r.Run("git", "clone", "--depth", "1", repoURL, repoDir); err != nil {
			return fmt.Errorf("git clone: %w", err)
		}
	}
	entries, err := s.fs.ReadDir(repoDir)
	if err != nil {
	...
	for _, e := range entries {
		...
		if fi, err := s.fs.Lstat(dest); err == nil && (fi.Mode()&fs.ModeSymlink) == fs.ModeSymlink {
			if link, err := s.fs.Readlink(dest); err == nil && link == src {
				logx.Info("dotfiles: skip link identical", "dest", dest)
				continue
			}
		}
		if err := s.fs.Symlink(src, dest); err != nil {
			return fmt.Errorf("symlink %s -> %s: %w", dest, src, err)
		}
		logx.Info("dotfiles: link created", "dest", dest, "src", src)
	}
	return nil
}
